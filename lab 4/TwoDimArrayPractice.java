    ...
    // Write your code here:
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        for (int column = 0; column < intArray[row].length; column++) {
	            ...
	            animate(row, column); // Animate the access
	        }
    ...
    // Write your code here:
	    if (row >= 0 && row < intArray.length) {
	        for (int column = 0; column < intArray[row].length; column++) {
	            ...
	            animate(row, column); // Animate the setting process
	        }
	    }
   ...
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	    if (column >= 0 && column < COLUMNS) {
	        int minimum = Integer.MAX_VALUE; // Initialize minimum with the highest possible int value

	        // Iterate through each row of the specified column to find the minimum value
	        for (int row = 0; row < ROWS; row++) {
	            if (intArray[row][column] < minimum) {
	                minimum = intArray[row][column]; // Update minimum if a smaller value is found
	            }
	            animate(row, column, minimum); // Animate the current state of finding minimum
	        }
	        return minimum; // Return the found minimum value
	    } else {
	        // Return some indication of invalid column or handle the error as per requirements
	        System.out.println("Column " + column + " is out of bounds.");
	        return -1; // Indicating an error condition or handle appropriately
	    }
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        for (int column = 0; column < intArray[row].length; column++) {
	            if (intArray[row][column] == value) {
	                count++;
	                animate(row, column); // Animate the discovery
	            }
	        }
	    }
